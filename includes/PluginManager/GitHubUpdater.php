...
namespace FluentCartSimulator\PluginManager;
...
class GitHubUpdater
{
    ...
    public function __construct($pluginFile, $githubRepo, $version)
    {
        ...
        $this->githubRepo = $githubRepo; // e.g. 'shamim0902/fluent-cart-simulator'
        $this->pluginName = 'FluentCart Order Simulator';
        $this->transientKey = 'fcsim_github_update_' . md5($this->slug);
    ...
    public function pluginInfo($data, $action, $args)
    {
        ...
        return (object) [
            ...
            'sections'      => [
                'description' => 'Automatically generates realistic test orders for FluentCart on a configurable schedule.',
    ...
    private function getLatestRelease()
    {
        ...
        // Find the zip asset from release assets
        $downloadUrl = '';
    ...
    private function maybeDeleteTransient()
    {
        ...
        if (isset($_GET['fcsim-check-update'])) {
            if (current_user_can('update_plugins')) {
                ...
                wp_redirect(admin_url('plugins.php?s=fluent-cart-simulator&plugin_status=all'));
                exit();
            }
        }
    }
